...
""" % bcb_dev)
  ...
  xperience_version = target_info.GetBuildProp("ro.axolotl.build.version")
  build_id = target_info.GetBuildProp("ro.build.id")
  build_date = target_info.GetBuildProp("ro.build.date")
  security_patch = target_info.GetBuildProp("ro.build.version.security_patch")
  device = target_info.GetBuildProp("ro.ax.device")
  device_status = target_info.GetBuildProp("ro.ax.channeltype")
  branch = target_info.GetBuildProp("ro.build.version.qcom")
  model = target_info.GetBuildProp("ro.product.model")

  script.Print("----------------------------------------------");
  script.Print("          (C)2020 Axolotl Open Source        ");
  script.Print("               By Klozz Jesus                ");
  script.Print("           TeamMEX@xda-developers            ");
  script.Print("----------------------------------------------");
  script.Print(" Android version: %s"%(android_version));
  script.Print(" axolotl version: %s"%(xperience_version));
  script.Print(" Build id: %s"%(build_id));
  script.Print(" CAF Branch: %s"%(branch));
